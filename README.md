...
Every engineering project writes down three kinds of thing: what has to be
true, where it is built, and what proves it. They usually end up in three
separate documents that drift apart, and nobody notices until somebody asks
for the proof.

This template keeps them attached to each other. You write each of them as one
small Markdown file in an Obsidian vault — a requirement, the module that owns
it, the test run that checked it — and two dependency-free Python scripts read
those files back out: a validator that checks every file keeps its form, and an
exporter that reads the whole vault into a graph.

Here is that graph. Nobody drew it. The exporter emitted it from the worked
example this template ships with — a battery monitoring module — and CI redraws
it on every push and fails if it no longer matches the files:

<!-- traceability-graph:start -->
```mermaid
flowchart LR
  ...
  o_ARC_BAT_001["ARC-BAT-001<br>ARC_Battery_Monitoring"]
  o_CMP_BAT_001["CMP-BAT-001<br>CMP_Battery_Pack"]
  o_IFC_BAT_001["IFC-BAT-001<br>IFC_PWR_DC_LiPo_Pack"]
  o_TAE_BAT_001["TAE-BAT-001<br>TAE_Battery_Log_Acceptance"]
  r_REQ_BAT_001("REQ-BAT-001<br>proven")
  r_REQ_BAT_002("REQ-BAT-002<br>proven")
  r_REQ_BAT_003("REQ-BAT-003<br>proven")
  o_ARC_BAT_001 -->|"allocates: Verified"| r_REQ_BAT_001
  o_CMP_BAT_001 -->|"allocates: Verified"| r_REQ_BAT_003
  o_IFC_BAT_001 -->|"allocates: Verified"| r_REQ_BAT_002
  o_ARC_BAT_001 -->|"evidence: Verified"| o_TAE_BAT_001
  o_CMP_BAT_001 -->|"evidence: Verified"| o_TAE_BAT_001
  o_IFC_BAT_001 -->|"evidence: Verified"| o_TAE_BAT_001
  o_TAE_BAT_001 -->|"verifies"| r_REQ_BAT_001
  o_TAE_BAT_001 -->|"verifies"| r_REQ_BAT_002
  o_TAE_BAT_001 -->|"verifies"| r_REQ_BAT_003
```
<!-- traceability-graph:end -->

Follow one requirement through it. `REQ-BAT-002` says a recorded voltage stays
inside the operating range. An interface contract owns it — that is the
`allocates` arrow. The same row that allocates it points at an evidence note,
the `evidence` arrow. And the note names the requirement back, the `verifies`
arrow. Three files, written separately, and a triangle that closes: that is the
whole method, and it is the only thing the tools insist on.

The exporter counts how many of those triangles are closed:

<!-- traceability-counts:start -->
```text
requirements: 3  proven: 3  not proven: 0
objects: 7  relations: 20  findings: 0
```
<!-- traceability-counts:end -->

`proven: 3` is a count and not a claim. It is the number of requirements whose
allocation row reached `Verified` **and** whose evidence note names them back,
computed from the vault rather than remembered — the same rule that put the
word `proven` on the three nodes above. The same run writes
`traceability.html`, the report you would hand to a reviewer, and `0 not
proven` is the sentence that report exists to be able to stop saying.

Run it yourself, on this repository, in one command:
...
why this repository keeps almost none. The exceptions are the blocks on this
page marked as generated: CI regenerates the export on every push and fails if
any of them has drifted from the vault.
...
- **A traceability exporter** that reads the vault into a graph and writes it
  out as a report, two CSV views, a JSON graph, a compact Markdown index and
  the Mermaid diagram at the top of this page — both directions of the
  requirement-to-evidence matrix, with what is unproven stated rather than
...
The graph at the top of this page draws the three relations that decide
coverage, and leaves out the four that do not. The same export writes
`traceability_index.md`, which leaves nothing out: one line per object and per
requirement, identifier, domain, file, and the first sentence of its own
context. That is the file an agent reads first, and it is what the seven
objects of this thread look like written out.

<details>
<summary>The worked example as the exporter indexes it — 7 objects, 3 requirements</summary>
